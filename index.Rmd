---
title: "Najważniejsze wydarzenia sesji na warszawskiej giełdzie na godz. 10:05 w dziewięciu wykresach"
...
#### 1. Sytuacja w Warszawie na tle największych rynków
...
bp5<-ggplot(data=stock3,aes(x=time,y=change,group=name))+
  ...
  theme(plot.title = element_text(color = "#EC0108",size=19,face="bold"))+
  ...
  labs(title="",
       subtitle=paste0("Indeksy giełdowe w dniu dzisiejszym na godzinę ",godzina,", zmiana procentowa od zamknięcia dnia poprzedniego (notowania 5min)"),
...
bp3<-ggplot(data=m4,aes(group=theme,fill=color))+
  ...
  coord_flip(clip="off")+
  geom_text(data=m4,aes(spolka,ratio+sign(ratio)*spread/14,label=label,color=color))+
...
bp3b<-ggplot(data=mc4,aes(group=theme,fill=color))+
  ...
  scale_fill_manual(values = cols)+
  coord_flip(clip="off")+
  geom_text(data=mc4,aes(spolka,ratio+sign(ratio)*spread/14,label=label,color=color))+
  scale_color_manual(values = cols)+
  facet_wrap(~theme,scales="free")+
  theme(strip.background = element_rect(fill="transparent",color="transparent"))+
  theme(strip.text = element_text(size=11, face="bold",color="gray23"))+
